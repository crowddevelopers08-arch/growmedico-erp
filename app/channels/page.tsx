...
function AudioPlayer({ src, isMine }: { src: string; isMine: boolean }) {
  ...
  return (
    <div className={cn("flex w-[min(70vw,220px)] sm:w-auto items-center gap-2 rounded-xl px-3 py-2 min-w-0 sm:min-w-[180px]", isMine ? "bg-primary-foreground/10" : "bg-muted/60")}>
      <audio
        ref={audioRef}
        src={src}
      ...
      />
      <Button variant="ghost" size="icon" className="size-8 shrink-0 rounded-full bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => {
...
function AttachmentChip({ att, isMine }: { att: Attachment; isMine: boolean }) {
  if (att.type.startsWith("image/")) {
    return <a href={att.data} download={att.name} className="block"><img src={att.data} alt={att.name} className="max-w-[min(65vw,200px)] max-h-48 rounded-lg object-cover" /></a>
  }
  return (
    <a href={att.data} download={att.name} className={cn("flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-opacity hover:opacity-80", isMine ? "bg-primary-foreground/10" : "bg-muted/60")}>
...
function ChannelsPageContent() {
  ...
  const loadChannels = useCallback(async () => {
    ...
    const groups = data.filter((channel) => channel.kind !== "direct")
    setChannels(groups)
    setSelectedChannel((prev) => prev ? groups.find((channel) => channel.id === prev.id) ?? null : groups[0] ?? null)
  }, [])

  const loadMessages = useCallback(async (channelId: string, append = false) => {
    const url = append && lastMessageTime.current
  ...
  const handleDeleteChannel = async () => {
    ...
    const res = await fetch(`/api/channels/${deleteChannel.id}`, { method: "DELETE" })
    if (!res.ok) {
  ...
  const groupedMessages = messages.map((msg, i) => {
    ...
    const isFirst = showDateSeparator || !prev || prev.senderId !== msg.senderId || new Date(msg.createdAt).getTime() - new Date(prev.createdAt).getTime() > 5 * 60 * 1000
  ...
  return (
    <div className="flex h-[calc(100dvh-7.2rem)] sm:h-[calc(100dvh-8rem)] w-full min-w-0 overflow-hidden rounded-none sm:rounded-xl border-0 sm:border border-border/40 shadow-none sm:shadow-md">
      <div className={cn("w-full sm:w-72 flex flex-col bg-background border-r border-border/30 sm:flex", showList ? "flex" : "hidden sm:flex")}>
        ...
        <div className="flex-1 overflow-y-auto bg-background">
          {channels.length === 0 ? (
            ...
            channels.map((ch) => (
              <div
                key={ch.id}
                onClick={() => { setSelectedChannel(ch); setShowList(false) }}
                className={cn("flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors border-b border-border/20 group", selectedChannel?.id === ch.id ? "bg-primary/10" : "hover:bg-accent/50")}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">{ch.lastMessagePreview ?? ch.description ?? "Channel"}</p>
                </div>
              </div>
            ))
          )}
      ...
      <div className={cn("flex-1 flex flex-col min-w-0", showList ? "hidden sm:flex" : "flex")}>
        {selectedChannel ? (
          <>
            <div className="flex h-14 items-center gap-1.5 border-b bg-card px-2 shadow-sm shrink-0 sm:gap-3 sm:px-4">
              <Button variant="ghost" size="icon" className="size-8 text-muted-foreground hover:text-foreground sm:hidden" onClick={() => setShowList(true)}>
                          ...
                          <div className={cn("flex flex-col max-w-[92%] sm:max-w-[70%] lg:max-w-[60%]", isMine ? "items-end" : "items-start")}>
                            <div className="relative group">
                              {(canEdit || canDelete) && !isEditing && (
                                <div className={cn("absolute -top-7 flex items-center gap-0.5 bg-background border border-border rounded-md px-1 py-0.5 shadow-md opacity-0 group-hover:opacity-100 transition-opacity z-10", isMine ? "right-0" : "left-0")}>
                                  {canEdit && <Button variant="ghost" size="icon" className="size-5" onClick={() => { setEditingMsgId(msg.id); setEditContent(msg.content) }}><Pencil className="size-3" /></Button>}
                                  {canDelete && <Button variant="ghost" size="icon" className="size-5 hover:text-destructive" onClick={() => setDeleteMsgId(msg.id)}><X className="size-3" /></Button>}
                                </div>
                              )}
                              {isEditing ? (
                                <div className="space-y-1.5 w-[min(16rem,80vw)]">
                                  <Textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} onKeyDown={(e) => {
                                    if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void handleEditSave(msg.id) }
                                    if (e.key === "Escape") setEditingMsgId(null)
                                  }} rows={2} className="text-sm resize-none" autoFocus />
          ...
          <div className="grid gap-4 py-2">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Input placeholder="general, announcements..." value={channelName} onChange={(e) => setChannelName(e.target.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""))} />
              </div>
            </div>
            <div className="space-y-2">
      ...
      <AlertDialog open={!!deleteChannel} onOpenChange={(o) => !o && setDeleteChannel(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => void handleDeleteChannel()} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={!!deleteMsgId} onOpenChange={(o) => !o && setDeleteMsgId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => deleteMsgId && void handleDeleteMessage(deleteMsgId)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
